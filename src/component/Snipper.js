...
class Snipper extends React.Component {
  ...
  initCropper(e) {
    ...
    snipWindow.loadURL(path.join('http://localhost:3000/') + '?snip')
    ...
    this.handleStream = (stream) => {
      ...
      video_dom.onloadedmetadata = function () {
        ...
        try {
          ...
          console.log(
            'Error Detroying connect to stream  , func getScreenshot',
            e
          )
        }
      }
  ...
  updateCropperCordinates = (cord) => {
    ...
    if (cord) {
      ...
      let pixelsToAddInYBaseOnErrorInY = (cord.x * errorInY) / 100
      ...
      setTimeout(() => {
        this.getScreenShot((base64data) => {
          ...
          Jimp.read(encondedImageBuffer, (err, image) => {
            if (err) throw err
            console.log(
              'Cordinates originaly from rnd for x=0 , y=0 =>',
              coordinates
            )
            let updatedCord = this.updateCropperCordinates(coordinates)

            console.log('Updated cord => ', updatedCord)

            let crop = updatedCord
              ? image.crop(
                  updatedCord.x + 24,
                  updatedCord.y + 44,
                  updatedCord.width,
  ...
  createNotification() {
    ...
    let notification = new electron.remote.Notification({
      ...
      body: 'Screen Shot has been taken Successfully',
